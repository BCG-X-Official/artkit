...
class OllamaChat(ChatModelConnector[AsyncOpenAI], metaclass=ABCMeta):
    """
    Base class for connection to Ollama LLMs.
    """
    ...
    def get_default_api_key_env(cls) -> str:
        """Ollama requires no API key since it's a self-managed server."""
        return "EMPTY"

    def _make_client(self) -> AsyncOpenAI:  # pragma: no cover
        """
        This method handles the authentication and connection to the Ollama server.
        Since Ollama implements the OpenAI API spec, the OpenAI client
        can connect to it.
        """
        return AsyncOpenAI(api_key="EMPTY", base_url=self.ollama_url)
